...
void generate(Ui::MainWindow *ui, const pwdgen::Options &options) {
    ::visualizeOptions(ui, options);

    auto text = pwdgen::generate(options).value_or(ERROR_MSG);
    ui->passwordEdit->setText(QString::fromStdString(text));
}
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(std::make_unique<Ui::MainWindow>()) {
    ...
    ::visualizeOptions(ui.get(), pwdgen::MEDIUM_OPTIONS);

    // Connect actions
    connect(ui->actionGenerate, &QAction::triggered, [this]() {
        ::generate(ui.get(), pwdgen::Options{
    ...
    connect(ui->actionUNIX, &QAction::triggered, [this]() { ::generate(ui.get(), pwdgen::UNIX_OPTIONS); });
    connect(ui->actionPIN, &QAction::triggered, [this]() { ::generate(ui.get(), pwdgen::PIN_OPTIONS); });
    connect(ui->actionMedium_Password, &QAction::triggered, [this]() { ::generate(ui.get(), pwdgen::MEDIUM_OPTIONS); });
    connect(ui->actionLong_Password, &QAction::triggered, [this]() { ::generate(ui.get(), pwdgen::LONG_OPTIONS); });
